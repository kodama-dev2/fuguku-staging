<?php
/*
 * Plugin Name: Acowebs Woocommerce Dynamic Pricing Pro
 * Version: 5.1.7
...
define('AWDP_WC_PRODUCT_VARIATIONS', 'product_variation');

define('AWDP_TOKEN', 'awdp');
define('AWDP_PRO_CHECK', true);
define('AWDP_VERSION', '5.1.7');
...
define('AWDP_TEXT_DOMAIN', 'aco-woo-dynamic-pricing');// dont use this MACRO, directly use wcpa-text-domain
...
function awdp_pro_activation()
{
    if (in_array('aco-woo-dynamic-pricing/start.php', apply_filters('active_plugins', get_option('active_plugins')))) {
        deactivate_plugins(WP_PLUGIN_DIR . '/aco-woo-dynamic-pricing/start.php');
        if (in_array('aco-woo-dynamic-pricing/start.php', apply_filters('active_plugins', get_option('active_plugins')))) {
            $free_version_name = get_plugin_data(WP_PLUGIN_DIR . '/aco-woo-dynamic-pricing/start.php');
...
register_activation_hook(__FILE__, 'awdp_pro_activation');
