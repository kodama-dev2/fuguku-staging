...
class AWDP_viewPricingTable
{
    ...
    { 
        ...
        foreach ( $rules as $rule ) {
            ...
            $rules_to_validate          = ['cart_total_amount', 'cart_total_amount_all_prods', 'cart_items', 'cart_items_all_prods', 'cart_products', 'cust_prev_order_count', 'cart_user_role', 'cart_user_selection', 'payment_method', 'shipment_method', 'number_orders', 'amount_spent', 'last_order', 'previous_order', 'product_in_cart'];
            $cart_total                 = 0;
            ...
            if ( $quantity_type == 'type_cart' ) { 
                ...
                */
                if ( $table_layout == 'horizontal' ) {
                    $table .= '<div class="wdp_table_outter"><h4>' . $awdp_pc_title . '</h4><table class="wdp_table lay_horzntl" data-table="'.str_replace('"','\'',json_encode($quantity_rules)).'" data-product="'.$item->get_id().'" data-price="'.$price.'" data-var-price="'.$var_price_display.'"><tbody class="wdp_table_body">';
                    ...
                    if ( $value_display == 'discount_both' ) {
                        $table .= '<div class="wdp_table_outter"><h4>' . $awdp_pc_title . '</h4><table class="wdp_table" data-table="'.str_replace('"','\'',json_encode($quantity_rules)).'" data-product="'.$item->get_id().'" data-price="'.$price.'" data-var-price="'.$var_price_display.'"><thead><tr class="wdp_table_head"><td>' . $awdp_qn_label . '*' . '</td><td>' . $awdp_pc_label . '</td><td>' . $awdp_nw_label . '</td></tr></thead><tbody class="wdp_table_body">';
                    } else {
                        $table .= '<div class="wdp_table_outter"><h4>' . $awdp_pc_title . '</h4><table class="wdp_table" data-table="'.str_replace('"','\'',json_encode($quantity_rules)).'" data-product="'.$item->get_id().'" data-price="'.$price.'" data-var-price="'.$var_price_display.'"><thead><tr class="wdp_table_head"><td>' . $awdp_qn_label . '*' . '</td><td>' . $awdp_pc_label . '</td></tr></thead><tbody class="wdp_table_body">';
                    }
                }
                ...
                */ 
                ...
                if ($table_layout == 'horizontal') {
                    $table .= '<div class="wdp_table_outter"><h4>' . $awdp_pc_title . '</h4><table class="wdp_table lay_horzntl" data-table="'.base64_encode(json_encode($quantity_rules)).'" data-product="'.$item->get_id().'" data-price="'.$price.'" data-var-price="'.$var_price_display.'"><tbody class="wdp_table_body">';
                    $tr_qn = '<tr><td>' . $awdp_qn_label . '</td>';
                    $tr_pr = '<tr><td>' . $awdp_pc_label . '</td>';
                    if ( $value_display == 'discount_both' ) {
                        $tr_nw = '<tr><td>' . $awdp_nw_label . '</td>';
        ...
        if ( $table != '' && ( ( !$tiered_pricing && $quantity_type == 'type_product' ) || ( $tiered_pricing && $quantity_type != 'type_product' ) ) ) {
