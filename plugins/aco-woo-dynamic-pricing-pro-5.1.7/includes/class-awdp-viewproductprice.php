...
class AWDP_viewProductPrice
{
    ...
    public function product_price ( $rules, $price, $item_id, $product, $prodLists, $cartRules, $item_price, $display_price, $skip_items )
    {
        ...
        $priceDisplayType               = get_option('awdp_variation_display') ? get_option('awdp_variation_display') : '';
        ...
        if ( $rules != false && sizeof ( $rules ) >= 1 ) { 
            ...
            foreach ( $rules as $rule ) {
                ...
                $checkUsage = call_user_func_array ( 
                    array ( new AWDP_Discount(), 'check_usage_restrctions' ), 
                    array ( $rule ) 
                );
                ...
                // // Skip Rule 
                // if ( $prod_ID && isset($rule['apply_rule_once']) && $rule['apply_rule_once'] && in_array ( $prod_ID, $skip_items ) && $rule['type'] != 'gift' ) {
                //     continue;
                // }

                // Skip Rule 
                if ( $prod_ID && isset($rule['apply_rule_once']) && $rule['apply_rule_once'] && in_array ( $prod_ID, $skip_ids ) && $rule['type'] != 'gift' ) {
                    continue;
                }

                if ( ( $rule['type'] == 'percent_product_price' || $rule['type'] == 'fixed_product_price' ) ) {

                    $badgeText = ( $rule['type'] == 'fixed_product_price' ) ? wc_price ( $dymcDisc ) . ' OFF' : $dymcDisc . '% OFF';
            ...
            if ( $ProductRuleActive ) { 
                ...
                if ( $variationCheck ) { 
                    ...
                    if ( $priceDisplayType == 'highest' && $max_price > 0 ) {

                        $item_price = wc_format_sale_price($max_price * $converted_rate, $maxdiscountprice * $converted_rate).$suffixText;

                    } else if ( $priceDisplayType == 'lowest' && $min_price > 0  ) {
...
//                                 $item_price = '<p class="price"><del><span class="woocommerce-Price-amount amount"><span class="woocommerce-Price-currencySymbol">'.wc_format_price_range( $min_price, $max_price ).'</del> <ins><span class="woocommerce-Price-amount amount"><span class="woocommerce-Price-currencySymbol">'.wc_format_price_range( $mindiscountprice, $maxdiscountprice ).'</ins></p>';
            ...
            // $badgeText                      = $badgeText ? '<span class="wdpBadge"> ('.$badgeText.')</span>' : '';
            // $item_price                     = $item_price . $badgeText; 
