...
class Cekongkir_Shipping_Method extends WC_Shipping_Method {
	...
	public function __construct( $instance_id = 0 ) {
		...
		$this->id                 = CEKONGKIR_METHOD_ID;
		$this->method_title       = cekongkir_get_plugin_data( 'Name' );
		$this->title              = cekongkir_get_plugin_data( 'Name' );
		$this->method_description = cekongkir_get_plugin_data( 'Description' );
	...
	public function init_form_fields() {
		if ('ID' !== WC()->countries->get_base_country()) {
			$this->instance_form_fields = array(
				'error' => array(
					'title'       => __('Error', 'cekongkir'),
					'type'        => 'title',
					'description' => __('This plugin only works for Store Address based in Indonesia.', 'cekongkir'),
				),
			);
		...
		$settings = array(
			'origin_location_destination' => array(
				'title'       => __('Origin Location Destination', 'cekongkir'),
				'type'        => 'select',
				'description' => __('Search and select an origin for shipping.', 'cekongkir'),
				'class'       => 'wc-enhanced-select',
				'options'     => $saved_id ? array($saved_id => $saved_label) : array(), // Set saved value if available
				'default'     => $saved_id,
			),
			'sort_shipping' => array(
				'title'       => __('Sort Shipping', 'cekongkir'),
				'type'        => 'select',
				'default'     => 'no',
				'options'     => array(
					'cost'      => __('By Cost - Ascending', 'cekongkir'),
					'cost_desc' => __('By Cost - Descending', 'cekongkir'),
					'name'      => __('By Name - A to Z', 'cekongkir'),
					'name_desc' => __('By Name - Z to A', 'cekongkir'),
					'no'        => __('No', 'cekongkir'),
				),
				'description' => __('Sort the shipping couriers list in the cart and checkout page.', 'cekongkir'),
			),
			'show_eta' => array(
				'title'       => __('Show ETA', 'cekongkir'),
				'label'       => __('Yes', 'cekongkir'),
				'type'        => 'checkbox',
				'description' => __('Show estimated time of arrival during checkout.', 'cekongkir'),
			),
			'base_weight' => array(
				'title'             => __('Base Cart Contents Weight (gram)', 'cekongkir'),
				'type'              => 'number',
				'description'       => __('The base cart contents weight will be calculated. If the value is blank or zero, the couriers list will not be displayed when the actual cart contents weight is empty.', 'cekongkir'),
			...
			'selected_couriers' => array(
				'title'   => __('Couriers', 'cekongkir'),
				'type'    => 'selected_couriers',
				'restore' => true,
			),
		);
	
		// Handle couriers based on account features and zones
		$features = array_merge(
			$this->api->get_zones(),
			Cekongkir_Account::get_features_label()
		);
		...
		foreach ($features as $feature_key => $feature_label) {
			...
			foreach ($accounts as $type => $account) {
				if (in_array($feature_key, array('domestic', 'international'), true)) {
					...
					$account_features['value'][$type] = $account->feature_enable($feature_key) ? __('Yes', 'cekongkir') : __('No', 'cekongkir');
				}
			}
	
			$settings['selected_couriers']['features'][$feature_key] = $account_features;
		}
	 ...
	 */
	 ...
	 public function register_hooks() {
		// Restore instance settings values
		add_filter('woocommerce_shipping_' . $this->id . '_instance_settings_values', array($this, 'restore_instance_settings_values'), 10, 2);
	
		wp_localize_script('cekongkir-select2', 'cekongkir_select2_params', array(
			'ajax_url' => admin_url('admin-ajax.php'),
			'nonce'    => wp_create_nonce('cekongkir_nonce'),
			'saved_origin' => array(
				'id'   => get_option('origin_location_destination', ''),
				'text' => get_option('origin_location_label', '')
			)
		));
	 ...
	 * @param Cekongkir_Shipping_Method $instance Instance class object.
	 *
	 * @return mixed
	 */
	public function instance_option_mapping( $option, $key, $instance ) {
		...
		switch ( $key ) {
			...
			// case 'api_key':
			// 	$api_key_hardcoded = apply_filters( 'cekongkir_api_key_hardcoded', false );
...
<tr valign="top">
    <th scope="row" class="titledesc">
        <label for="<?php echo esc_attr( $field_key ); ?>"><?php echo wp_kses_post( $data['title'] ); ?>
            <?php echo $this->get_tooltip_html( $data ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></label>
    </th>
    <td class="forminp">
...
<tr valign="top">
    ...
    <td class="forminp">
        <ul class="cekongkir-account">
            <?php foreach ( $this->api->get_accounts() as $account ) { ?>
            <li class="cekongkir-account-item">
                <label for="<?php echo esc_attr( $field_key ); ?>--<?php echo esc_attr( $account->get_type() ); ?>"
                    class="cekongkir-account-item-label cekongkir-account-item-label--<?php echo esc_attr( $account->get_type() ); ?>">
                    <?php echo esc_html( $account->get_label() ); ?>
                </label>
                <input type="radio" name="<?php echo esc_attr( $field_key ); ?>"
                    value="<?php echo esc_attr( $account->get_type() ); ?>"
                    id="<?php echo esc_attr( $field_key ); ?>--<?php echo esc_attr( $account->get_type() ); ?>"
                    class="cekongkir-account-type" <?php checked( $account->get_type(), $this->get_option( $key ) ); ?>
                    <?php echo $this->get_custom_attribute_html( $data ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
            </li>
            <?php } ?>
        </ul>
		...
		foreach ( $this->api->get_zones() as $zone_id => $zone_label ) {
			...
			if ( ! empty( $selected_couriers[ $zone_id ] ) ) {
				foreach ( $selected_couriers[ $zone_id ] as $courier_id => $selected_services ) {
					...
					$website         = ! empty( $courier['website'] ) ? add_query_arg( 'utm_source', 'cekongkir.com', $courier['website'] ) : '';
...
<tr valign="top">
    ...
    <td class="forminp">
        <div class="cekongkir-tab-container">
            <ul class="cekongkir-tab-nav">
                <?php foreach ( $zones as $zone_id => $zone ) : ?>
                <li>
                    <a href="#cekongkir-tab-content-<?php echo esc_attr( $zone_id ); ?>"
                        class="cekongkir-tab-nav-item cekongkir-tab-nav-item--<?php echo esc_attr( $zone_id ); ?>">
            ...
            <?php foreach ( $zones as $zone_id => $zone ) : ?>
            <div id="cekongkir-tab-content-<?php echo esc_attr( $zone_id ); ?>"
                class="cekongkir-tab-content cekongkir-tab-content--<?php echo esc_attr( $zone_id ); ?>">
                <ul class="cekongkir-couriers" id="cekongkir-couriers-<?php echo esc_attr( $zone_id ); ?>">
                    <?php foreach ( $zone['couriers'] as $courier_id => $courier ) : ?>
                    <li class="cekongkir-couriers-item" data-id="<?php echo esc_attr( $courier_id ); ?>"
                        data-zone="<?php echo esc_attr( $zone_id ); ?>">
                        <div class="cekongkir-couriers-item-inner">
                            <div class="cekongkir-couriers-item-info">
                                <label class="cekongkir-couriers-item-info-title">
                                    <input type="checkbox" class="cekongkir-service cekongkir-service--bulk"
                                        <?php checked( $courier['selected'], true ); ?> />
                                    <span
                                        class="cekongkir-couriers--label"><?php echo wp_kses_post( $courier['label'] ); ?></span>
                                    <span
                                        class="cekongkir-couriers--selected"><?php echo esc_html( $courier['count_selected'] ); ?></span>
                                    <span
                                        class="cekongkir-couriers--available"><?php echo esc_html( $courier['count_available'] ); ?></span>
                                </label>
                                <?php if ( $courier['website'] ) : ?>
                                <div class="cekongkir-couriers-item-info-link">
                                    <a href="<?php echo esc_url( $courier['website'] ); ?>" target="blank"
                                        title="<?php esc_attr_e( 'Visit courier\'s website', 'cekongkir' ); ?>">
                                        <span class="dashicons dashicons-admin-links"></span>
                                    </a>
                                </div>
                                <?php endif; ?>
                                <div class="cekongkir-couriers-item-info-toggle">
                                    <a href="#" class="cekongkir-couriers-toggle"
                                        title="<?php esc_attr_e( 'Toggle', 'cekongkir' ); ?>">
                                        <span class="dashicons dashicons-admin-generic"></span>
                                    </a>
                                </div>
                            </div>
                            <ul class="cekongkir-services">
                                <?php foreach ( $courier['services'] as $service_id => $service ) : ?>
                                <li class="cekongkir-services-item">
                                    <label>
                                        <input type="checkbox" class="cekongkir-service cekongkir-service--single"
                                            name="<?php echo esc_attr( $field_key ); ?>[]"
                                            value="<?php echo esc_attr( $service['value'] ); ?>"
                                            <?php checked( $service['selected'], true ); ?>>
...
<?php
	...
	public function validate_account_type_field( $key, $value ) {
		...
		if ( empty( $value ) ) {
			...
			throw new Exception( wp_sprintf( __( '%s is required.', 'cekongkir' ), $field_label ) );
		}

		if ( ! $this->api->get_account( $value ) ) {
			// translators: %s is setting field label.
			throw new Exception( wp_sprintf( __( '%s is invalid.', 'cekongkir' ), $field_label ) );
		}
	...
	public function validate_selected_couriers_field( $key, $value ) {
		...
		if ( ! $value ) {
			...
			throw new Exception( wp_sprintf( __( '%1$s is required.', 'cekongkir' ), $field_label ) );
		}
		...
		if ( $account ) {
			foreach ( $value as $zone => $couriers ) {
				...
				// 	throw new Exception( wp_sprintf( __( '%1$s: Account type %2$s is not allowed to select multiple couriers.', 'cekongkir' ), $field_label, $account->get_label( 'label' ) ) );
				// }
				...
				// 	throw new Exception( wp_sprintf( __( '%1$s Shipping: Account type %2$s is not allowed to select courier_data %3$s.', 'cekongkir' ), $field_label, $account->get_label( 'label' ), strtoupper( implode( ', ', array_keys( $not_allowed ) ) ) ) );
				// }
			}
		}
	...
	public function calculate_shipping( $package = array() ) {
		try {
			...
			$cache_enable = cekongkir_is_enable_cache();
			 ...
			 * @param cekongkir_Shipping_Method $object  Current class object.
			 *
			 * @return array
			 */
			$results = apply_filters( 'cekongkir_shipping_results', $results, $package, $this );
			...
			if ( ! $results ) {
				throw new Exception( __( 'No couriers data found', 'cekongkir' ) );
			}

			if ( ! is_array( $results ) ) {
				// translators: % s Encoded data response .
				throw new Exception( wp_sprintf( __( 'Couriers data is invalid: %s', 'cekongkir' ), wp_json_encode( $results ) ) );
			}
			...
			foreach ( $results['parsed'] as $result ) {
				 ...
				 * @return string
				 */
				$label = apply_filters( 'cekongkir_shipping_rate_label', $label, $result, $package, $this );
	...
	private function get_origin_info( $shipping_address = array() ) {
		...
		$selected_origin = get_option('origin_location_destination', 0);
	...
	private function calculate_shipping_api_request_data( $package = array() ) {
		try {
			if ( ! isset( $package['destination']['country'] ) ) {
				throw new Exception( __( 'Shipping destination country is empty.', 'cekongkir' ) );
			}
			 ...
			 * @return array
			 */
			$origin_info = apply_filters( 'cekongkir_shipping_origin_info', $this->get_origin_info( $package['destination'] ), $package );

			if ( empty( $origin_info ) ) {
				throw new Exception( __( 'Shipping origin info is empty or invalid', 'cekongkir' ) );
			}
			 ...
			 * @return array
			 */
			$destination_info = apply_filters( 'cekongkir_shipping_destination_info', $this->get_destination_info( $package['destination'] ), $package );

			if ( ! $destination_info || ! array_filter( $destination_info ) ) {
				throw new Exception( __( 'Shipping destination info is empty or invalid', 'cekongkir' ) );
			}

			error_log(sprintf('cekongkir_shipping_destination_info: %s', wp_json_encode($destination_info)));
			 ...
			 * @return array
			 */
			$dimension_weight = apply_filters( 'cekongkir_shipping_dimension_weight', $this->get_dimension_weight( $package['contents'] ), $package );

			if ( ! $dimension_weight || ! array_filter( $dimension_weight ) ) {
				throw new Exception( __( 'Cart weight or dimension is empty or invalid', 'cekongkir' ) );
			}
			...
			if ( ! $courier || ! array_filter( $courier ) ) {
				throw new Exception( __( 'No couriers selected', 'cekongkir' ) );
			}
	 ...
	 */
	 ...
	 public function get_destination_info($shipping_address = array()) {
		if (!isset($shipping_address['country'])) {
			return false;
		}
		...
		if ('international' === $zone) {
			$country = cekongkir_get_json_data(
				'country',
				array(
			...
			return array(
				'destination' => $country['id'], // Use country ID as destination for international
			);
		}
	
		// If Indonesia, use the selected destination
		$selected_destination = WC()->session->get('selected_destination_id', null);
		
		error_log(sprintf('selected_destination_id: %s', $selected_destination));
		if (!$selected_destination) {
			$selected_destination = get_option('selected_destination_id', 0);
		}
	
		error_log(sprintf('selected_destination_id: %s', $selected_destination));
	
		return array(
			'destination' => intval($selected_destination),
		);
	}
	...
	/**
 * Handle AJAX request to fetch destinations.
 */


/**
 * Save the shipping destination to the order meta.
 *
 * @param int $order_id Order ID.
 */
public function save_shipping_destination($order_id) {
    if (!empty($_POST['shipping_destination'])) {
        update_post_meta($order_id, '_shipping_destination', sanitize_text_field($_POST['shipping_destination']));
    }
}


/**
 * Display the shipping destination in the admin order page.
 *
 * @param WC_Order $order Order object.
 */
public function display_shipping_destination_in_admin($order) {
    $destination = get_post_meta($order->get_id(), '_shipping_destination', true);
    if ($destination) {
        echo '<p><strong>' . __('Shipping Destination') . ':</strong> ' . esc_html($destination) . '</p>';
    }
}
