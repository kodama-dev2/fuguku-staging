        ...
        function createWPMegamenuMetaBox(){
          ...
          $check_menu_count = count ( $menulocations );
           ...
           $menu_general_settings = get_option( 'wpmegabox_settings' );
            if ( ! $check_menu_count  ) { ?>
           <p><?php _e("To Enable WP Mega Menu Pro, First please assign this menu to theme location.<br/>This Menu is not assigned to any theme location yet.", APMM_PRO_TD);?></p>
           <?php  }else if(!count($get_all_registered_menu_locations)){ ?>
           <p><?php _e("Please create a new menu location in order to activate WP Mega Menu Pro. This Theme doesnot have any menu location created yet.", APMM_PRO_TD);?></p>
           <?php }else{ 
    ...
    /* Pro Features added */
      ...
      */
      function wpmegamenu_custom_menu_items_meta_box() {
            if (wp_get_nav_menus()) {
                add_meta_box('wpmm_pro_advanced_menu_items', __('WPMM Pro Advanced Menu Items', APMM_PRO_TD), array($this, 'wpmm_custom_menu_items_meta_box'), 'nav-menus', 'side', 'low');
            }
        }
